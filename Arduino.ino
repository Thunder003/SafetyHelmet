...
const int buzzer = 9;
boolean jerk, sleep;
float sensitivity = 8000;

void setup() {
  Wire.begin(); 
  pinMode(buzzer, OUTPUT);//Start I2C as master
...
void loop(){
 ...
 //Subtract the offset values from the raw gyro values
  ...
  Serial.print(" | Acceleration  = "); Serial.println(acc_total_vector);
  Serial.print(" Jerk = "); Serial.println(jerk);
  if(acc_total_vector > sensitivity)
   {
    jerk = true;
    }

    if(millis()>5000 && (abs(angle_pitch_output)>35 || abs(angle_roll_output+87.79)>35))
   {
    sleep = true;
   }
